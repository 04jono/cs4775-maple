...
def probVectTerminalNode(diffs,tree,node):
	...
	for entry in diffs:
		...
		#when the alignment is the same as the reference, nothing is recorded
		#thus we need to check if the diff_pos is greater than the reference_pos
		#which inidcates to us that there is an R entry
		if diff_pos > reference_pos: 

			#in the paper the second entry keeps track of the start of the entry
			#but in the code it keeps track of the end

			probVect.append((4,diff_pos-1))
			reference_pos = diff_pos
		
		if diff_type == 'n' or diff_type == '-':
			#diff_pos and ref_pos should be the same 
			probVect.append((5,diff_pos + conseq_pos-1))
			reference_pos = diff_pos + conseq_pos

		elif diff_type in allelesLow: #the diff_type is A,G,C, or T

			#In all other cases the second element represents the mutation-annotated 
			# tree reference nucleotide at the considered position at the considered node.
			if allelesLow[diff_type]==refIndeces[diff_pos-1]:
				#if the bases match by accident then they should be type R

				probVect.append((4,diff_pos))
			else:
				tree_ref_nucl = refIndeces[diff_pos-1]
				probVect.append((allelesLow[diff_type],tree_ref_nucl))
			reference_pos=diff_pos + 1

		else:
			if onlyNambiguities:
				probVect.append((5,diff_pos))
			else:
				tree_ref_nucl = refIndeces[diff_pos-1]
				if usingErrorRate and( numMinSeqs == 0):
					...
					#using the default error rates provided in ambiguities
					probVect.append((6,tree_ref_nucl,ambiguities[diff_type]))
			reference_pos = diff_pos + 1

		#if we have traversed through the end of the alignment and we haven't
		#reached the end of the reference sequence then the remainder is made of 
		#R entries
	if reference_pos<=lRef:
			...
			if type_1 < 4:
				...
				for i in range(4):
					if onlyFindIdentical:
						...
						if entry1[-1][i]!=entry2[-1][i]:
							return 0
					elif entry1[-1][i] < 0.1 and entry2[-1][i] > 0.1:
							moreInfo1 = True
					elif entry1[-1][i] > 0.1 and entry2[-1][i] < 0.1:
							moreInfo2 = True
				ref_pos+=1
...
def appendProbNode(probVectP,probVectC,isTipC,bLen):
	...
	while ref_pos < lRef:
		...
		#score so we just need to update the positions of our reference and
		...
		if childEntry[0]==5 or parentEntry[0]==5:
			...
			if childEntry[0]==5:
				...
				#the parent entry is either A,G,T,C, or O
				else:
					#move on to the next reference position
					ref_pos+=1
					if ref_pos==lRef:
						break
					parentIndex +=1
					parentEntry = probVectP[parentIndex]
				if childEntry[1]==ref_pos:
					childIndex+=1
					childEntry = probVectC[childIndex]
			
			#the parent entry is N
			elif parentEntry[0]==5:

				#the child entry is R (it can't be N since we checked that in the 
				#previous branch)
				if childEntry[0]==4:
					ref_pos=min(parentEntry[1],childEntry[1])
					if ref_pos==lRef:
				...
				#the child entry is A,G,T,C or O)
				else:
					ref_pos+=1
					if ref_pos==lRef:
						break
				...
				print('Error: parent or child should have been type N')
		
		#Child entry and parent entry are of the same type: {R,A,C,G,T}
		else:

			#did not reimplement this, involves using errorRate for branch lengths
			#updates the branch lengths to compute the likelihoods
			#this is equivalent to the l2 + c1 addition in the paper
			if parentEntry[0]!=childEntry[0] or parentEntry[0]==6:
				contribLength=bLen
				if parentEntry[0]<5:
			...
			#The third bullet point entry types are not the same are of one of the types: {R,A,C,G,T}
			elif childEntry[0] != parentEntry[0] and childEntry[0] != 6 and parentEntry[0] != 6:
			...
			#This is the last bullet point, where at least one of the entries is type O
				...
				if childEntry[0]==6 and parentEntry[0]==6:
					if useRateVariation:
						mutMatrix=mutMatrices[ref_pos]
					tot=0.0
					if contribLength:
